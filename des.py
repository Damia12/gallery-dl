...
def descargar_linux(url):
    ...
    # ── FIX BUG 6: Inicialización defensiva antes del bloque TRY ──
    master_fd = None
    proceso = None
    buffer = ""
    ultimo_output = time.time()
    ...
    try:
        # Envolvemos la creación del PTY y del Proceso dentro del try
        master_fd, slave_fd = pty.openpty()
        proceso = subprocess.Popen(
            cmd, stdout=slave_fd, stderr=slave_fd, close_fds=True, env=env_vars
        )
        os.close(slave_fd)  # Cerramos el lado esclavo en el padre de inmediato
        ...
        # ── FIX BUG 6: Validaciones de existencia en el bloque de salida ──
        ...
        sys.stdout.write(
            "\033[?25h"
        )  # Garantiza devolver el cursor visible a la terminal
        sys.stdout.flush()
